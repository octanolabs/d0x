...
export default {
  ...
  computed: {
    methods () {
      return this.$store.state.apis[this.apiId] ? this.$store.state.apis[this.apiId].openrpc.document[this.openrpcType].deref.methods : []
    },
    selected () {
  ...
  methods: {
    ...
    prevOperation () {
      this.$store.commit('setSelected', { apiId: this.apiId, method: this.selected - 1 })
    },
    nextOperation () {
      this.$store.commit('setSelected', { apiId: this.apiId, method: this.selected + 1 })
    }
